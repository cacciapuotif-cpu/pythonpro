# SESSIONE 2026-07-17 — Apertura contesto PythonPro (Codex)

## Fatto
- Letto lo stato obbligatorio Yellow Lion e aperto il percorso attivo `/DATA/progetti/pythonpro` partendo da questo `STATUS.md`, senza riesplorare il repository.
- Confermato lo stato corrente: Wave 2.1 chiusa e attivata, migration `056` a head, ultima suite completa 423 passed/1 skipped, backend e frontend healthy.
- Nessuna modifica applicativa, nessun dato modificato, nessun restart e nessun push.

## Pendente principale
- Il collaboratore test con email `codex.runtime.test.20260715@example.invalid` causa `ResponseValidationError` nella lista collaboratori; la bonifica resta subordinata ad autorizzazione esplicita.
- Vincoli invariati: preservare il worktree sporco, non fare push e non attivare `AGENT_DATA_RETENTION_ENABLED` senza approvazione.

---

# SESSIONE 2026-07-17 — Diagnosi accesso LAN porta 3001 (Codex)

## Esito
- Diagnosi in sola lettura, nessun riavvio o modifica applicativa.
- Tutti i container PythonPro sono attivi; frontend e backend risultano healthy.
- Frontend pubblicato correttamente su `0.0.0.0:3001`; richieste locali a `http://127.0.0.1:3001/` e `http://192.168.2.41:3001/` restituiscono HTTP 200.
- Bundle JavaScript `main.ab2469df.js` raggiungibile con HTTP 200; backend `http://192.168.2.41:8001/health` restituisce `{"status":"ok"}`.
- IP `192.168.2.41` confermato locale all'host tramite routing kernel. Firewall `ufw`, `firewalld` e `nftables` inattivi.
- Nei log Nginx non compare una richiesta recente proveniente dal browser dell'utente: il blocco avviene prima dell'applicazione, verosimilmente sul percorso di rete del dispositivo (Wi-Fi/LAN differente, dati mobili/VPN o client isolation). Un accesso iPhone precedente risultava riuscito alle 13:47 UTC tramite un indirizzo/referer `172.22.0.1:3001`.
- Diagnosi estesa dopo errore Windows `ERR_CONNECTION_TIMED_OUT`: `eth0` e' UP con `192.168.2.41/24`, gateway `192.168.2.1`; ZeroTier e' attivo su `ztx6n6z6sk` con IP server `172.22.0.1/16`. `http://172.22.0.1:3001/` restituisce HTTP 200. Se Windows non si trova realmente nella LAN `192.168.2.0/24` ma e' collegato alla rete ZeroTier, deve usare `http://172.22.0.1:3001/`.
- Browser Windows via ZeroTier raggiunge il frontend, ma l'API viene bloccata da CORS. Preflight reale con origin `http://172.22.0.1:3001` restituisce HTTP 400 `Disallowed CORS origin`; la `.env` contiene l'origine LAN `http://192.168.2.41:3001` ma non quella ZeroTier. Preflight di controllo per l'origine LAN restituisce HTTP 200 con header `access-control-allow-origin` corretto. Nessun fix applicato in questa fase diagnostica.
- Fix CORS ZeroTier autorizzato e applicato: backup `.env.bak_20260717_cors_zerotier`, aggiunta origine `http://172.22.0.1:3001` a `CORS_ALLOWED_ORIGINS`, ricreato soltanto il container backend. Verifiche post-recreate: backend e frontend healthy; preflight ZeroTier HTTP 200 con `access-control-allow-origin: http://172.22.0.1:3001`; GET `/health` HTTP 200 `{"status":"ok"}` con header CORS; log startup senza errori e migration 056 invariata/completata. Nessun rebuild frontend, nessuna modifica DB applicativa.
- Conferma browser post-fix: frontend mostra `Connessione API stabilita`; login admin raggiunge il backend ma restituisce HTTP 401, quindi il problema residuo e' solo la credenziale. La password monouso generata il 2026-07-14 non e' registrata nei file (solo hash nel DB) e non e' recuperabile. Nessun reset effettuato senza nuova autorizzazione esplicita.
- Su autorizzazione utente, password monouso dell'account `admin` (user id 1) rigenerata tramite `backend/reset_password.py` e verificata immediatamente con login reale `POST /api/v1/auth/login`: HTTP 200. Password comunicata solo in chat e non registrata nello status.

## Prossimo controllo
- Dal dispositivo verificare di essere sulla stessa rete `192.168.2.x`; altrimenti, se ZeroTier e' connesso, provare `http://172.22.0.1:3001/`. Per diagnosi Windows acquisire `ipconfig`, `ping 192.168.2.41` e `Test-NetConnection 192.168.2.41 -Port 3001`.
- Accesso atteso via ZeroTier: `http://172.22.0.1:3001/`; effettuare hard refresh del browser e verificare login. Gli avvisi `contentscript.js`/`ObjectMultiplex` sono attribuibili a un'estensione browser e non al frontend PythonPro.
