...
import { Collaboratore } from '@/src/types/api';
import { colors, spacing, typography, borderRadius, shadows } from '@/src/styles/tokens';
import { API_BASE_URL } from '@/src/lib/constants';

export default function CollaboratoriScreen() {
  const [data, setData] = useState<Collaboratore[]>([]);
  ...
  const loadData = useCallback(async (isRefresh = false) => {
    try {
      ...
      console.log(`📡 Fetching collaboratori from: ${API_BASE_URL}`);
      const result = await api.getCollaboratori();
      setData(result);
      console.log(`✅ Loaded ${result.length} collaboratori`);
    } catch (err) {
      console.error('❌ Error loading collaboratori:', err);
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.card} activeOpacity={0.7}>
            <View style={styles.cardContent}>
              ...
              {item.ruolo && (
                <Text style={styles.cardSubtitle}>{item.ruolo}</Text>
              )}
              {item.email && (
