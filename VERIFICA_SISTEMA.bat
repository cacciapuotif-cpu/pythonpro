...
docker exec pythonpro_db psql -U admin -d gestionale -c "SELECT 1;" >nul 2>&1
if %errorlevel% equ 0 (
...
docker exec pythonpro_db psql -U admin -d gestionale -c "\d collaborators" | findstr "documento_identita_filename" >nul 2>&1
if %errorlevel% equ 0 (
...
docker logs pythonpro_backend --tail 20 2>&1 | findstr /I "ERROR CRITICAL"
echo.
echo --- Frontend (ultimi 20 righe) ---
docker logs pythonpro_frontend --tail 20 2>&1 | findstr /I "ERROR Failed"
